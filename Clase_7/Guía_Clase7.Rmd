...
### **Ejercicio 7.** **Graficar con la función ggplot ()** 

Con conjunto de datos **datos1** realice un boxplot donde la variable X es Year y en el eje Y =**ProdNac**, colore por Especie. Volver la variable Especies Factor y la variable Year entera.
```{r datos1, fig.width= 8}
 ggplot(datos1, aes(x= Especies, y= ProdNac, color=Year))+
   geom_point()+
...
### **Ejercicio 8.** **Importar la base de Datos2** 
...
### **Ejercicio 9.** **Reemplazar los valores faltantes en la base de datos datos2** 
...
### **Ejercicio 10.** **Genere una nueva variable a partir de la información de otras variables** 

Genere una nueva variable llamada **NTotal** a partir de la suma de los datos de las variables Region_Lagos, Region_Aysen y Region_Magallanes de la base de datos **datos2** generada en el **ejercicio 9**. También, genere una variable llamada **Per_NTotal** que es el porcentaje mensual de casos de ISA confirmados. Use la función **_mutate_**.
...
### **Ejercicio 11.** **Realice un diagrama de barras** 

Cargue la base de datos **datos2_new.txt**, estos datos son los que se generaron con la información del ejercicio anterior. Use ggplot() para realizar un diagrama de barras con la variables X = Mes, e Y = NTotal.
...
```{r datos2_new, fig.width= 10}
datos2_new<-read.table("datos2_new.txt", header = TRUE, sep = " ")
datos2_new
datos2_new[,1] <- as.factor(datos2_new$Mes)
datos2_new
mes_cod<- datos2_new[sort(order(datos2_new$Mes)),] 
levels(mes_cod) <- c("Enero","Febrero","Marzo","Abril","Mayo","Junio",                               "Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre")
mes_cod
...
ggplot(Datos_new, aes(x=Mes, y= NTotal))+
  geom_col() +
  expand_limits(y=0)+
  labs(title= "Número de nuevos casos (incidencia) confirmados HPR0 durante el 2019", y= "Nº Total de casos de ISA por mes", x= "Mes",
       caption = "Fuente: Sernapesca")+
  guides(fill=FALSE, color=FALSE)+
  theme_bw()
```
